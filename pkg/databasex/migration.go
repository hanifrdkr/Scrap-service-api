...
func DatabaseMigration(cfg *Config) {
	...
	//dbstring := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?parseTime=true", cfg.User, cfg.Password, cfg.Host, cfg.Port, cfg.RoleName)

	fn, ok := dsn[cfg.Driver]
	if !ok {
		log.Fatal(fmt.Sprintf("invalid driver %s", cfg.Driver))
	}
	...
	defer func() {
		if err := db.Close(); err != nil {
			log.Fatal(fmt.Sprintf("db migrate: failed to close DB: %v\n", err))
		}
	}()
